...
TASKS=${TASKS:-8}
TESTS_INPUT=`pwd`/nadav_tests/inputs
TESTS_GLOB=$TESTS_INPUT/${1:-test_*}
TESTS_OUTPUT=`pwd`/nadav_tests/outputs
TESTS_EXPECTED=`pwd`/nadav_tests/expected
FORK_PRELOAD=`pwd`/nadav_tests/runner/preload/fork_preload.so
CLEANER="python3 `pwd`/nadav_tests/runner/output_cleaner.py"
SMASH=`pwd`/smash
RUNNER=`pwd`/nadav_tests/runner/runner
...
rm -rf $TMP_FOLDER
cp -r ./nadav_tests/required_folder $TMP_FOLDER
cd $TMP_FOLDER
